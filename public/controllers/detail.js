  ...
  .controller('DetailCtrl', ['$scope', '$rootScope', '$routeParams', 'Show', 'Subscription',
    function($scope, $rootScope, $routeParams, Show, Subscription) {
      Show.get({ _id: $routeParams.id }, function(show) {
        ...
        // Alternative to using a for loop by using the native JS .filter() method
        $scope.nextEpisode = show.episodes.filter(function(episode) {
          return new Date(episode.firstAired) > new Date();
        })[0]; // we may get an array, depending on first aired dates. however, we are interested only in the first one, hence the zero index
      });
    }
  ]);